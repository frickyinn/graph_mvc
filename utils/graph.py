...
# ====== Graph construction
def graph_computing(adj_coo, cell_num, k=20, knn_distance_type='euclidean'):
    edgeList = []
    for node_idx in range(cell_num):
        tmp = adj_coo[node_idx, :].reshape(1, -1)
        distMat = distance.cdist(tmp, adj_coo, knn_distance_type)
...
def graph_construction(adj_coo, cell_N, k=20, knn_distance_type='euclidean', using_mask=False):
    adata_Adj = graph_computing(adj_coo, cell_N, k, knn_distance_type)
